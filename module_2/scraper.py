...
def _load_progress():
    """
    Loads the last successfully completed survey page
    """
...
def _save_progress(last_completed_page, next_page, next_url, records):
    """
    Saves the pagination progress; I save the next page so that
    restarting the program doesn't re-process the most recently
    completed page
    """
    ...
    temp_file = progress_file_name + ".tmp"
...
def _commandeer_chrome(port=chrome_port):
    """
    Initializes a Selenium instance and attaches it to
...
def _fetch_pages_in_browser(driver, urls):
    ...
    performst he requests using fetch(), which preserves
...
def _process_batch(driver, batch, existing_urls):
    ...
    for result in fetched:
        ...
        if not html:
            print(f"FAILED: {result_url} (HTTP {result.get('status')})")
            continue
...
def scrape_data(survey_url, target_records=target_records):
    """
    This function loads previously scrapped data and scraping
    ...
    print(f"Remaining: {max(0, target_records - len(data)):,}")

    progress = _load_progress()

    if progress and progress.get("next_url"):
        current_url = progress["next_url"]
        page_number = progress.get("next_page", 1)
    ...
    try:
        while len(data) < target_records:
            ...
            for start in range(0, len(new_results), batch_size):
                ...
                print(f"TOTAL: {len(data):,}/{target_records:,}")
                save_data(data)
            ...
            _save_progress(
                last_completed_page=previous_page,
            ...
            print()
            print(f"Page {previous_page} completed")
            print(f"Progress saved")
            print(f"Next run will start at page {page_number}")
...
if __name__ == "__main__":
    scrape_data(survey_url, target_records=target_records)
    print()
    print("Done!")